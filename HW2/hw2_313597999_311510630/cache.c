...
cache_rows* get_to_cache_updating_LRU(cache_board* C, uint32_t address){
    ...
    uint32_t ref = ((C->bit_extent_ref)-1) & offset;
    uint32_t tag = address >> (C->previous_blockExtent);
    tag = tag >> ((uint32_t)(log(C->bit_extent_ref) / log(2)));
    int32_t  current_assoc = match_line_to_tag(tag, C,ref);
    print_num++;
    if (current_assoc != EMPTY_LINE)
    {
        print_num++;
        LRU_update(ref, tag,C);
        return C->row[ref*C->assoc + current_assoc];
    }
...
static cache_rows* find_first_invalid(cache_board* C,uint32_t ref,bool* no_invalid)
{
    for (uint32_t cur_assoc = 0; cur_assoc < C->assoc;cur_assoc++)
    {
        cache_rows* current_row = C->row[ref*C->assoc+cur_assoc];
...
void update_new_block (cache_rows* new_line,uint32_t address,uint32_t tag,uint32_t ref)
{
