...
void hendelInst(MT_cpu_t * cpu,int curthread, int progIndex)
{
    ...
    if(inst.opcode == CMD_ADD || inst.opcode == CMD_SUB){
        ...
        if(inst.opcode == CMD_LOAD){
            mem_calc = cpu->threads[curthread].context.reg[inst.src1_index];
            mem_calc += (inst.isSrc2Imm)? (inst.src2_index_imm) : (cpu->threads[curthread].context.reg[(inst).src2_index_imm]);
            SIM_MemDataRead(mem_calc, &cpu->threads[curthread].context.reg[inst.dst_index]);
        }
        else{
            mem_calc = cpu->threads[curthread].context.reg[inst.dst_index];
            mem_calc += (inst.isSrc2Imm)? inst.src2_index_imm : cpu->threads[curthread].context.reg[(inst).src2_index_imm];
            SIM_MemDataWrite(mem_calc, cpu->threads[curthread].context.reg[inst.src1_index]);
        }
